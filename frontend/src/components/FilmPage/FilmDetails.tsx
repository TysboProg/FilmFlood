...
export default function FilmDetails({ film }: any) {
    ...
    const cleanedGenres = film.genres ? film.genres.map((genre: string) => cleanText(genre)) : []
    const cleanedCountries = film.countries ? film.countries.map((country: string) => cleanText(country)) : []
        ...
        <div className="bg-[#1c1c24] rounded-xl shadow-xl overflow-hidden" style={audiowide.style}>
            <div className="flex flex-col md:flex-row items-center md:items-start">
                ...
                <div className="p-4 md:p-8 w-full md:w-2/3 lg:w-3/4">
                    ...
                    <p className="mt-2 text-gray-400 text-sm md:text-base">{renderValue(film.description)}</p>

                    <div className="flex items-center gap-3 mt-3">
                        <div className="bg-[#ffcc00] text-black font-bold px-2 py-1 rounded text-sm">{film.rating}</div>
                        <div className="text-white/70 text-sm border border-white/20 px-2 py-1 rounded">{film.ageRating}</div>
                        <div className="text-white/70 text-sm border border-white/20 px-2 py-1 rounded">{film.watchTime} мин</div>
                    </div>
            ...
            <div className="p-4 md:p-8 border-t border-[#2a2a36] mt-4">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                    {film.actors &&
                        film.actors.map((actor: any, index: any) => (
                            <div
                                key={index}
